    ...
    public class Program
    {
        ...
        public static void Main(string[] args)
        {
            using (NativeWindow nativeWindow = NativeWindow.Create())
            {
                ...
                var vertexShader2 = CreateVertexShaderShader();
                var program2 = Gl.CreateProgram();
                Gl.AttachShader(program2, vertexShader2);
                Gl.LinkProgram(program2);
                Gl.UseProgram(program2);

                int posAttrib = Gl.GetAttribLocation(program2, "aPosition");
                Gl.VertexAttribPointer((uint)posAttrib, 2, VertexAttribType.Float, false, 0, IntPtr.Zero);
        ...
        private static void NativeWindow_Render(object sender, NativeWindowEventArgs e)
        {
            ...
            Matrix4x4f model = new Matrix4x4f();

            Gl.BindVertexArray(_CubeVao);

            Gl.UniformMatrix4f(_CubeEdgeProgram_Location_uMVP, 1, false, ref model);
